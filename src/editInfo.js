...
import { uploadImage, addCompanyBasic, get_info, editCompanyBasic, isAuthenticated } from "./database";
import { useLocation } from "react-router-dom";

const FileInputImage = ({ onImageUpload, img }) => {
  const [image, setImage] = useState(img || default_img); // Define a imagem inicial como 'img' ou a imagem padrão
  ...
  const handleFileChange = (event) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        if (reader.result) {
          setImage(reader.result); // Atualiza a imagem local
          if (onImageUpload) onImageUpload(file); // Envia o arquivo para o backend
        }
      };
  ...
  return (
    <div style={{ textAlign: "center" }}>
      ...
      <label htmlFor="fileInput" style={{ position: "relative", display: "inline-block" }} tabIndex="0">
        <img
          src={image || img} // Usando a imagem local ou a imagem padrão
...
const EditInfo = () => {
  ...
  useEffect(() => {
    ...
    const handleImageUpload = (file) => {
      ...
      reader.onloadend = () => {
        if (reader.result) {
          setImg(reader.result); // Atualiza o estado com a URL da imagem
        }
      };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const docId = await editCompanyBasic(userId, company, url_res, desc, categoria, loc, face, whatsapp, telegram, instagram, tel);
    ...
    <div className="form_info">
      <form onSubmit={handleSubmit}>
        ...
        <p className="input_social"><i className="fa-brands fa-instagram fa-2xl" style={{ color: "#ebb42c" }}></i>
          <input type="text" placeholder="(opcional)" id="insta" value={instagram} onChange={(e) => setInsta(e.target.value)} />
        </p>
